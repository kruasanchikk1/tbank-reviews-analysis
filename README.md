...
1. **Скорость обработки обращения связана с недовольством** (по детрактор-флагу — сильно, rho = −0.47), но при проверке через официальный `csat_score` эффект почти исчезает (rho = 0.05) — вероятно, `csat_score` считается на узкой, уже эскалированной подвыборке. Не стоит обещать бизнесу линейный эффект от простого ускорения SLA.
2. **Удовлетворённость монотонно растёт вместе с ценностью клиентского сегмента** — доля детракторов падает с 46.4% (Защищённые) до 20.8% (Высокодоходные с ПМ), ANOVA F = 131.6, p < 0.001. Похоже на эффект уровня обслуживания, а не «более обеспеченные мягче».
3. **Разница между источниками отзывов значима, но умереннее, чем показала ошибочная первая версия анализа.** banki.ru — 35%, sravni.ru — 20%, otzovik — 75%, «пульс» — 51%. Разные площадки — разная природа канала, но без экстремумов вроде «100% негатива».
4. **Охват поддержки различается по направлениям бизнеса в разы**: от 0.9% решённых жалоб (mobile) до 74% (sme). При этом приоритет для бизнеса разный в зависимости от метрики: депозитные продукты дают наибольший **абсолютный объём** нерешённых жалоб (просто по масштабу направления), а mobile — это отдельная **процессная** проблема (мало жалоб, но почти ни одна не доходит до статуса «решено»).
5. **Новые клиенты (стаж < 3 месяцев) заметно недовольнее** действующих (50.5% против 39.5% детракторов). Судя по темам жалоб (чаще — карты, акции, погашение кредитов; реже — кэшбек, платежи), дело не в объективно худшем продукте, а в трении первого касания — практический вывод в пользу усиленной заботы о клиенте в первые недели как ставки на LTV.

6. **Облака слов по тексту отзыва подтверждают H1 с другой стороны.** У промоутеров доминируют «спасибо», «быстро», «оперативно», «поддержка» — язык благодарности за скорость. У детракторов — «деньги», «счёт», «вопрос», «ответ», «почему», «обращение» — язык ожидания ответа, а не жалоб на продукт как таковой.

Что проверялось, но не подтвердилось: влияние возраста клиента на оценку — проверил дважды (в среднем по выборке и отдельно внутри конкретных продуктов), эффекта не нашёл нигде, кроме одной наводки на выборке в 10-30 человек, которой недостаточно для вывода.
